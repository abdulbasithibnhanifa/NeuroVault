...
export class VectorStoreService {
  ...
  /**
   * Performs a similarity search against the vector store using cosine similarity.
   * @param queryEmbedding The embedding vector of the search query.
   * @param userId The user ID to scope the results to.
   * @param topK The number of candidate chunks to return.
   * @param matchThreshold The minimum cosine similarity score (e.g., 0.1).
   */
  async similaritySearch(queryEmbedding: number[], userId: string, topK: number = 5, matchThreshold: number = 0.05) {
    try {
      logger.info("Executing Supabase match_embeddings RPC", { userId, topK, matchThreshold });
      ...
      if (error) {
        logger.error("Error executing match_embeddings", { error: error.message, userId });
        throw error;
      }

      return data || [];
    } catch (error: any) {
      logger.error("Error in similaritySearch", { error: error.message, userId });
      throw error;
    }
  }
